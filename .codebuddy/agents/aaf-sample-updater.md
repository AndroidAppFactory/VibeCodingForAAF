...
```
---
📊 统计：更新 X 个文件 | 配置变更 Y 项，依赖变更 Z 项，UI 同步 W 个 | 编译：成功/失败
```
...
每次执行完成后，将摘要追加到 `{workspace}/.codebuddy/cache/aaf-sample-updater/history.log`：

```
[日期] [项目:Template-XXX] AAF [旧版本] → [新版本] | 文件 X 个，依赖 Y 项 | 编译：成功/失败 | 耗时约 Ns
```

## 负面反馈记录（E3 数据飞轮）

当用户指出升级问题（如版本号错误、遗漏文件、编译失败根因）时，将反馈追加到 `{workspace}/.codebuddy/cache/aaf-sample-updater/corrections.log`：
