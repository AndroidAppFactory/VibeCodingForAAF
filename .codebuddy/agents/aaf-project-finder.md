...
```
---
📊 统计：查找 X 个项目 | 找到 Y 个，未找到 Z 个 | 搜索路径 W 个
```
...
```
验证不通过的项目标记为 `⚠️ 路径存在但项目结构无效`。

## 历史归档（E2 记忆与复盘）

每次执行完成后，将摘要追加到 `{workspace}/.codebuddy/cache/aaf-project-finder/history.log`：

```
[日期] 查找 X 个项目 → 找到 Y 个 | 路径摘要：AAF=[path], Template-AAF=[path], ...
```

## 负面反馈记录（E3 数据飞轮）

当用户指出路径错误（如找到的不是正确项目、遗漏项目）时，将反馈追加到 `{workspace}/.codebuddy/cache/aaf-project-finder/corrections.log`：

```
[日期] [类型:路径错误/遗漏项目] 用户反馈：[项目] 正确路径应为 XXX
```

执行前**应读取** `corrections.log`（如存在），优先使用历史确认的正确路径。
