...
用户请求
    ↓
【读取历史记录】（必须执行）
├─ 读取 $WORK_ROOT/temp/cache/aaf-sample-upgrade/corrections.log（若存在），最近 5 条
├─ 读取 $WORK_ROOT/temp/cache/aaf-sample-upgrade/history.log（若存在）
└─ 有纠正记录时，优化调度策略
    ↓
...
提供提交建议 → 全部完成
    ↓
【记录日志】（必须执行）
├─ 成功 → 写入 $WORK_ROOT/temp/cache/aaf-sample-upgrade/history.log
└─ 用户纠正/取消 → 写入 $WORK_ROOT/temp/cache/aaf-sample-upgrade/corrections.log
```
...
|------|------|----------------|------------------------|---------------------------|
| AAF 模块版本查找 | 必须 | `aaf-dev/aaf_version` | `rules/aaf/aaf_version.mdc` | `.codebuddy/rules/aaf_version.mdc` |
| AAF 依赖管理规范 | 必须 | `aaf-dev/aaf_dependency` | `rules/aaf/aaf_dependency.mdc` | `.codebuddy/rules/aaf_dependency.mdc` |
| AAF Git Scope | 补充 | `aaf-dev/aaf_git` | `rules/aaf/aaf_git.mdc` | `.codebuddy/rules/aaf_git.mdc` |
...
```
---
[统计] 统计：升级 X 个项目 | 成功 Y 个，失败 Z 个 | 总计变更 W 个文件，M 项依赖 | 总耗时约 Ns
```
...
最终报告生成前，自动验证汇总数据的一致性：
- 对比每个 Agent 返回的编译结果与报告中的汇总是否一致
- 验证统计行中的"成功 Y 个，失败 Z 个"与实际 Agent 返回结果匹配
- 如有不一致，自行修正后再输出

## 人机协作增强（E7）

- 生成最终变更报告后，展示给用户确认，确认后再提供提交建议
- 如有项目升级失败，明确告知用户并提供错误信息，等待用户决策（重试/跳过/中止）
- 不自动对失败项目重试升级
...
当用户指出升级流程问题（如遗漏项目、Agent 调度错误、提交建议不当）时，将反馈追加到 `$WORK_ROOT/temp/cache/aaf-sample-upgrade/corrections.log`：

```bash
mkdir -p "$WORK_ROOT/temp/cache/aaf-sample-upgrade"
echo "[$(date '+%Y-%m-%d %H:%M')] 类型: {流程问题|遗漏|调度错误}
用户反馈: <XXX>
改进方案: <YYY>
---" >> "$WORK_ROOT/temp/cache/aaf-sample-upgrade/corrections.log"
```

执行前**应读取** `$WORK_ROOT/temp/cache/aaf-sample-upgrade/corrections.log`（如存在），优化调度策略。
